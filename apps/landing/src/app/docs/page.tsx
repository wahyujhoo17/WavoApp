...
const SideNavItem = ({ icon: Icon, label, id, isActive, onClick }: any) => (
  <button
    onClick={() => onClick(id)}
    className={`flex items-center gap-3 w-full px-4 py-2.5 rounded-xl text-[13px] font-bold transition-all cursor-pointer text-left ${
      isActive 
    ...
    <table className="w-full text-left min-w-[600px]">
      ...
      <tbody className="divide-y divide-white/[0.02]">
        ...
        <tr>
          ...
          <td className="px-6 py-3.5 text-[13px] text-[#8e8e93]">1,000</td>
...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'https://api-usewavo.lumicloud.my.id/api/v1';
...
export default function DocsPage() {
  ...
  const scrollTo = (id: string) => {
    ...
    if (element) {
      const yOffset = -90; // offset for the navbar
    ...
    <main className="min-h-screen bg-[#050505] text-on-surface selection:bg-primary selection:text-white">
      ...
      <div className="pt-24 flex min-h-screen max-w-container-max mx-auto">
        ...
        <div className="flex-1 max-w-[920px] p-6 md:p-12 mx-auto overflow-hidden">
          ...
          {/* ════════════════ GETTING STARTED ════════════════ */}
          <section id="getting-started" className="space-y-6 mb-14 animate-fade-in">
