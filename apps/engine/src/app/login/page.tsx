...
export default function LoginPage() {
  ...
  const [otpVal, setOtpVal] = React.useState<string[]>(new Array(6).fill(''));
  const inputRefs = React.useRef<(HTMLInputElement | null)[]>([]);

  // Update otpCode state whenever otpVal changes
  React.useEffect(() => {
    setOtpCode(otpVal.join(''));
  }, [otpVal]);

  const handleOtpChange = (element: HTMLInputElement, index: number) => {
    const value = element.value.replace(/\D/g, '');
    
    // Support typing when the block is already full (override value)
    if (value.length > 1) {
                  ...
                  <div className="flex justify-between gap-2 my-4">
                    {otpVal.map((data, index) => (
                      <input
                        ...
                        className="w-12 h-14 bg-black/40 border border-white/5 rounded-xl text-center text-xl font-bold text-white outline-none focus:border-primary/40 focus:ring-1 focus:ring-primary/20 transition-all font-mono"
                      />
                    ))}
                  </div>
                </div>
                ...
                <form className="space-y-5" onSubmit={handleLogin}>
                  ...
                  <div className="space-y-2 relative">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <Link href="/forgot-password" className="text-[11px] font-bold text-on-surface-variant/40 uppercase tracking-widest hover:text-primary transition-colors">Forgot Password?</Link>
                    </div>
